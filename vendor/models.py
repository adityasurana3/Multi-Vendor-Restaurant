...
class Vendor(models.Model):
    ...
    def save(self,*args, **kwargs):
        if self.pk is not None:
            ...
            context = {
                'user':self.user,
                'is_approved':self.is_apporoved
            }
            if orig.is_apporoved != self.is_apporoved:
                if self.is_apporoved == True:
...
HOURS_OF_DAY_24 = [(time(hour,min).strftime('%I:%M %P'),time(hour,min).strftime('%I:%M %P')) for hour in range(0,24) for min in (0,30)]
